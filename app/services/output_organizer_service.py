"""Post-download file organization: sorting saved images/metadata into folders.

`OutputOrganizerService` runs the three "Organize" actions available in the
UI after a download/process run: group by camera, group by Sol, or the
combined "simple layout" used by the default builder flow (camera and/or Sol,
toggled independently). All three operate on files already on disk in the
configured output folder -- they never touch the network or the catalog.
"""
...
class OrganizeResult:
    """Outcome of one organize operation, ready to display as a summary message."""
...
class OutputOrganizerService:
    """Moves already-downloaded images/metadata/masks into a tidy folder layout.

    Named like every other class in this package (`services/*_service.py`)
    even though its job is filesystem bookkeeping rather than a catalog/NASA
    operation -- it's instantiated and injected the same way as its siblings
    (see `actions.py::_get_output_organizer`).
    """
    ...
    def organize_source_buckets(self, out: Path) -> list[Path]:
        """Return the top-level folders under `out` that actually hold images.

        Splits work between the `PDS` and `RAW_PHOTOS` subfolders when they
        exist (so a PDS+RAW mixed download doesn't cross-contaminate their
        layouts), and falls back to `out` itself when images sit directly in
        the root or when neither subfolder exists.
        """
        buckets: list[Path] = []
        pds = out / "PDS"
        raw = out / "RAW_PHOTOS"
    ...
    def find_output_meta_for_image(self, src: Path, out_dir: Path) -> Optional[Path]:
        """Locate the `.meta.json` sidecar for image `src`, searching nearby first, then recursively."""
        candidates = [
    ...
    def _update_meta_outputs_meta_path(self, meta_path: Path) -> None:
        """After moving a `.meta.json` file, rewrite its own `outputs.meta_json_path` field to match its new location."""
    ...
    def extract_sol_for_output_file(self, src: Path, out_dir: Path) -> Optional[int]:
        """Determine which Sol `src` belongs to: meta.json first, then folder name, then filename prefix."""
    ...
    def organize_by_camera(self, output_path: str | Path) -> OrganizeResult:
        """Move every image under `output_path` into a `<camera>/` subfolder, keeping its meta.json alongside it."""
    ...
    def organize_simple_layout(
        ...
        *,
        ...
        """Organize the default (non-"organize by camera/Sol button") output layout.

        Runs after every download/process action, controlled by the
        "Divide by camera type" / "Divide by SOL" checkboxes. Independently
        of those flags, it always: relocates stray `.meta.json` files one
        level down into a `metadata/` subfolder, files MXYLF PDS mask
        products (`.img`/`.lbl`) into a `Masks/` subfolder, and keeps each
        image's mask/meta sidecar next to it after the move.
        """
        out = Path(output_path).expanduser()
        if not out.exists() or not out.is_dir():
            return OrganizeResult(False, self._t("output_path_required"))
        ...
        for root in scan_roots:
            ...
            try:
                for p in root.iterdir():
                    ...
                    sol_unknown = divide_by_sol and sol is None
                    if sol_unknown:
                        # Still organize by camera (if requested) instead of
                        # leaving the file completely unsorted in root just
                        # because its Sol couldn't be determined -- file it
                        # under "Unnamed" alongside whatever camera folder it
                    ...
                    if divide_by_sol:
                        sol_label = f"sol_{sol}" if sol is not None else "Unnamed"
                        dst_dir = dst_dir / sol_label
                        label_parts.append(sol_label)
                    dst_dir.mkdir(parents=True, exist_ok=True)
                    dst = dst_dir / p.name
                    try:
    ...
    def organize_by_sol(self, output_path: str | Path) -> OrganizeResult:
        """Move every image under `output_path` into a `sol_<N>/` subfolder (optionally nested under its camera folder)."""
        out = Path(output_path).expanduser()
        if not out.exists() or not out.is_dir():
            return OrganizeResult(False, self._t("output_path_required"))
        image_exts = {".jpg", ".jpeg", ".png"}
