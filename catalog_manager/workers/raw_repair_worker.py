"""The `repair` (RAW) worker: re-scans only the exact Sol locations a finished RAW
integrity check flagged as having missing products (via `--repair-locations-file`),
then installs the repaired catalog atomically. Driven by `jobs.start_raw_repair_job`.
See `pds_repair_worker.py` for the PDS-catalog counterpart -- the main structural
difference is that a single RAW manifest `catalog_url` covers every camera at a given
Sol, so locations here are keyed by Sol alone rather than by (Sol, collection).
"""
...
def run(job_id: str) -> int:
    """Run the repair: build the exact list of Sol locations to re-fetch from the integrity check's `missing_products`, re-scan just those via the RAW builder, compare before/after product counts and IDs, and install atomically. Returns a process-style exit code (`0` completed, `1` failed, `2` cancelled)."""
    ...
    source_json = PROJECT_ROOT / "data" / "catalog" / "Catalog_RawArch.json"
    source_parquet = PROJECT_ROOT / "data" / "catalog" / "Catalog_RawArch.parquet"
    ...
    try:
        ...
        # Unlike PDS (one Sol directory per camera), a single RAW manifest
        # catalog_url covers every camera at that Sol -- so only each
        # distinct Sol needs a fresh fetch, not one per product.
        locations_by_sol: dict[int, dict[str, Any]] = {}
        for item in missing_products:
            sol_url = str(item.get("sol_url") or "")
            sol = item.get("sol")
            if not sol_url or sol is None or int(sol) in locations_by_sol:
                continue
            locations_by_sol[int(sol)] = {"sol": int(sol), "sol_url": sol_url}
        target_locations = list(locations_by_sol.values())
        if not target_locations:
            raise RuntimeError("The referenced integrity check's missing products have no Sol location to repair.")
        ...
        if result == 2:
            ...
            return 2
        if result != 0:
            raise RuntimeError(f"RAW builder returned {result}")
        ...
        # else in the meantime, or turn out to be thumbnails/excluded by
        # current rules (see integrity_check_worker.py's remote_seen_ids cross-check).
        ...
        result_payload = {
            ...
            "catalog": "raw", "camera": camera, "integrity_job_id": integrity_job_id,
